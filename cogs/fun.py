...
class Fun(commands.Cog):
        ...
        if difficulty in levels.keys():
            ...
            while True:
                ...
                if guess < 1 or guess > range:
                    await ctx.send(f"Smh your number should be between **1** and **{range}**!")
                elif guess in guesses:
                    await ctx.send(f"You already picked that number. Maybe try something else?")
                else:
                    attempts += 1
                    if guess == my_number:
                        embed = nextcord.Embed(title="Guess the Number",
    ...
    async def roll(self, ctx, number: int, sides: int):
        if number > 10:
            await ctx.send("You can only roll up to 10")
        elif number < 1:
            await ctx.send("I can't roll nothing man")
        elif sides > 100:
            await ctx.send("hey i only have the ones up to 100 sides")
        elif sides < 4:
            await ctx.send("dice have at least 4 sides man")
        else:
            sum = 0
            dice = [
    ...
    async def roll_err(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            await ctx.send("Try again, `>roll <number> <sides>` is how you use this")
    ...
    async def choose(self, ctx, *, choices: str):
        if "|" in choices:
            ...
            choices = choices.split(" ")
        if len(choices) < 2:
            await ctx.send(f"Bruh, you only gave one choice, so I only have one option, `{choices[0]}` <:bruh:907585845798772766>")
        else:
            final = choice(choices)
            await ctx.send(f"**{ctx.author.name}**, I choose `{final}`!")

    @choose.error
    async def choose_err(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            await ctx.send(f"**{ctx.author.name}**, I choose `for you to run this command correctly`\n`>choose <choice 1,choice 2,...>` is how you use this")
